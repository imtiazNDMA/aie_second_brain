...
Development Editor: Melissa Potter Indexer: WordCo Indexing Services, Inc.
Acquisitions Editor: Jonathan Hassell Interior Designer: David Futato
Production Editor: Katherine Tozer Cover Designer: Susan Thompson
...
information. You can access this page at https://oreil.ly/prgrming-pytorch-for-dl.
...
[1, 1, 1],
[0, 0, 0]])
You can use special creation functions to generate particular types of tensors. In par‐
...
sors, but PyTorch prefers to deal with these in a [channel, height, width]. You can
...
tensor and a smaller tensor. You can broadcast across two tensors if, starting back‐
...
You can see how these rules are just going get more and more complicated to describe
...
You can run the download.py script in that directory, and it will download the images
...
much more quickly.
By default, PyTorch’s data loaders are set to a batch_size of 1. You will almost cer‐
...
memory. You may also want to experiment with some of the additional parameters:
...
is, it produces one result in a vector higher than everything else. You’ll often see it
...
You need to be aware of a couple of issues, though. The first is the danger of getting
...
You’ll see a way of determining a good learning rate in Chapter 4, but for now, you’ll
...
You’ve seen a lot of different pieces of code throughout this chapter, so let’s consoli‐
...
You have trained your first neural network—congratulations! You can now use it to
...
predictions on the test set that we created at the start of the chapter. You don’t need to
...
You might be a little disappointed with the accuracy of the SimpleNet architecture,
...
through each element of the array of layers in succession. You can use this to break
...
1 Kernel and filter tend to be used interchangeably in the literature. If you have experience in graphics process‐
...
fewer number of filters than they would if the 1 × 1 convolution wasn’t present. You
...
its simpler construction and the early availability of trained weights. You’ll often see it
...
mean centered around zero with a variance of 1. You might ask why we need to do
...
You might be wondering: if we have BatchNorm in our network, why are we normaliz‐
...
works work, including features such as Dropout, MaxPool, and BatchNorm. You’ve also
...
You might not want to freeze the BatchNorm layers in a model, as
...
data may end up being lost as BatchNorm corrects your input. You
...
In the preceding code, we take advantage of the in_features variable that allows us
to grab the number of activations coming into a layer (2,048 in this case). You can
...
Finally, we go back to our training loop and then train the model as per usual. You
...
You might remember from Chapter 2 that I introduced the concept of a learning rate
...
data presented in the training set rather than produce a generalized solution. You’ll
...
is simple. You may remember this snippet of code from Chapter 2:
transforms = transforms.Compose([
transforms.Resize(64),
transforms.ToTensor(),
transforms.Normalize(mean=[0.485, 0.456, 0.406],
...
You can specify a PIL resampling filter, and optionally provide an (x,y) tuple for the
...
on to your target 256 × 256 data. You’ll probably find a percentage point or two
...
where is the cat? You’d have a problem, because the network has no concept of
69

## Page 88

memory. This is incredibly important when it comes to dealing with data that has a
...
1 Note that it’s not impossible to do these things with CNNs; a lot of in-depth research in the last few years has
...
further information. And seq2seq!
...
time step and combined to update the shared parameters of the network. You can
...
end. You don’t have to follow the latest fad—honest!
biLSTM
...
A famous example of embedding vectors is word2vec, which was released by Google
...
You’ll also want to install spaCy (an NLP library), and pandas if you don’t have them
...
you want, and it’ll process the raw CSV (or JSON) for you. You do this by first defin‐
...
sequential Whether the field represents sequential data (i.e., text). If set to False, no True
tokenization is applied.
use_vocab Whether to include a Vocab object. If set to False, the field should contain True
numerical data.
init_token A token that will be added to the start of this field to indicate the beginning of the None
data.
eos_token End-of-sentence token appended to the end of each sequence. None
fix_length If set to an integer, all entries will be padded to this length. If None, sequence None
lengths will be flexible.
...
tokenize The function that will perform sequence tokenization. If set to spacy, the spaCy string.split
tokenizer will be used.
pad_token The token that will be used as padding. <pad>
unk_token The token that will be used to represent words that are not present in the Vocab <unk>
dict.
pad_first Pad at the start of the sequence. False
trun Truncate at the beginning of the sequence (if necessary). False
cate_first
...
Wait, wait, what? Yes, we specified 20,000, but by default, torchtext will add two
...
ber that a GPU gets its speed from operating on regular batches). You can also specify
...
on images. (You’ll see shortly that we have to update our training loop to deal with
...
Data Augmentation
You might wonder exactly how you can augment text data. After all, you can’t really
...
gies: random insertion, random swap, and random deletion. Let’s take a look at each
of them.3
Random Insertion
...
3 See “EDA: Easy Data Augmentation Techniques for Boosting Performance on Text Classification Tasks” by
...
You need to be aware of a few limits. First, you can translate only up to 15,000 charac‐
...
You might have noticed that everything I’ve said so far about augmentation hasn’t
...
Transfer Learning?
You might be wondering why we haven’t talked about transfer learning yet. After all,
...
with some data augmentation strategies for text-based data. You have plenty to
...
You may also want to alter the size of the embedding vector. Larger vectors mean that
...
look at PyTorch’s torchaudio library. You’ll learn how to use it to construct a pipeline
...
The ESC-50 dataset is a set of WAV files. You can download it either by cloning the
Git repository:
...
The function will read in our WAV files and MP3 files. You can also generate tensors,
...
We have one of those rare things, a perfectly balanced dataset. Let’s break out the
...
ing generating mel spectrograms (You’ll see what these are a little later in the chap‐
...
ber using it back in 1997 to convert WAV files into MP3s on my first ever Linux box.
...
1 Understanding all of what SoX can do is beyond the scope of this book, and won’t be necessary for what we’re
going to be doing in the rest of this chapter.
Exploring ESC-50 | 95
...
We now have the means for loading one of the WAV files from the ESC-50 dataset
...
The __init__ method takes the path parameter, finds all the WAV files inside that
...
You’ll see that it uses a lot of our building blocks from Chapter 3, but instead of using
...
architectures and their accuracy scores. You’ll notice that in comparison, we’re not
This Frequency Is My Universe | 99
...
custom dataset class that loads these images from the disk. (You can even add the
...
dataset that we’re turning into spectrogram images. You might be wondering whether
...
just with ResNet-50. You could try with ResNet-101 and perhaps an ensemble of dif‐
...
ting, don’t forget). Can we do the same here? Yes! In fact, there are two approaches
...
name of the effect, and an array of parameters to send to sox. You can get a list of
...
crop may cut out enough frequencies that it potentially changes the output class. This
...
3 See “SpecAugment: A Simple Data Augmentation Method for Automatic Speech Recognition” by Daniel S.
Park et al. (2019).
...
known to contain some incorrect labels. You should check yours, especially if cate‐
...
You could change the gradient, mask off activations, replace all the biases in the mod‐
...
start streaming into TensorBoard, as shown in Figure 7-5. You’ll have to switch to
...
You might ask, “What does this have to do with deep learning?” Fair enough; it’s a
...
pip install py-spy
You can find the process identifier (PID) of a running process and attach py-spy by
...
pretrained=True). You can click Reset Zoom to go back to the full flame graph. Also,
...
4GB! You can get the current PID of a notebook by using os.getpid(). It turns out
that the process using the most memory was actually an experimental notebook I was
using to test out the GPU transforms in the previous section! You can imagine that
...
You can start up a web server on the command line by setting the CATFISH_HOST and
...
image_url parameter to the /predict path. You should see a JSON response showing
...
the classification performance. Didn’t we spend a lot of time training it? Yes, we did,
...
of predictions across all classes. You could certainly return the pre‐
...
the idea of shipping containers. You specify a bundle of files (typically, using a Dock‐
...
specified and the programs you’ve told it to run. You can then share the Dockerfile so
...
which we use here to download the latest version of the image we’re after. You may
...
5000. You should be able to go back to http://localhost:5000/predict just as before.
...
of the size of our images comes from the size of the parameter file. You may not have
...
or Azure Blob Storage. You’ll have to use the respective provider’s
...
needs to be corrected. In an actual deployment, you’d include things like user_ids
...
1 Cloud Native DevOps with Kubernetes by John Arundel and Justin Domingus (O’Reilly) is a great deep dive
into this framework.
Deploying on Kubernetes | 147
...
--port 80
--target-port 5000
You can then look at the running services by using kubectl get services to get the
external IP:
kubectl get service
NAME CLUSTER-IP EXTERNAL-IP PORT(S) AGE
catfish-service 10.3.251.122 203.0.113.0 80:30877/TCP 3d
You should now be able to hit http://external-ip/predict just as you could on your
...
kubectl get pods
You’ll see that the pod we’ve specified has been deleted. But—you should also see that
...
Scripting
You might wonder why we just can’t trace everything. Although the tracer is good at
...
ture, we’d need to inherit from torch.jit.ScriptModule instead of nn.Module. You
...
trace through just that instead. You could implement the classifier block yourself
...
turn it into a Dockerized web service that can be deployed via Kubernetes. You’ve also
...
nies. You’ll also learn some new techniques along the way, including resizing pictures,
...
But how do we calculate the loss function of this mixed-up image? Well, if p is the
...
You can think of this as a layer that inverts a standard Conv2d transform (with its own
...
nn.ReLU(inplace=True))
Try it! You should find that the output tensor is twice as big as the input. If we have
...
GANs were introduced by Ian Goodfellow in a 2014 paper and are a novel way of
...
rectangle using x , x , y , y coordinates. Instead of just supplying images as training
1 2 1 2
...
3 See “U-Net: Convolutional Networks for Biomedical Image Segmentation” by Olaf Ronneberger et al. (2015).
...
You’ll find U-Net-based architectures cropping up all over Kaggle segmentation com‐
...
with different confidence_threshold values and different pictures. You can also
...
You’ll then need to add your newly created dataset to maskrcnn_benchmark/data/
...
on page 190 for more ideas, including the wonderfully entitled You Only Look Once
(YOLO) architecture. In the meantime, we look at how to maliciously break a model.
Adversarial Samples
You have probably seen articles online about images that can somehow prevent image
...
Black-Box Attacks
You may have noticed that to produce an image that fools the classifier, we need to
...
filter that allows in only images that pass some filtering tests. You could in theory
180 | Chapter 9: PyTorch in the Wild

## Page 199

make a neural net to do that too, because then the attackers have to try to break two
...
5 See “Neural Machine Translation by Jointly Learning to Align and Translate” by Dzmitry Bahdanau et al.
(2014).
More Than Meets the Eye: The Transformer Architecture | 181
...
In the groundbreaking paper “Attention Is All You Need,”6 Google researchers pointed
...
and how to get images, text, and audio to flow through them as tensors. You should
...
• You Only Look Once (YOLO), a family of fast object detection models with
...
Index
Symbols audio (see sound)
@app.route() function, 140 autoencoder, 168
@torch.jit.script_method, 154 AutoML, 44
__call__, 64, 110 AWS (Amazon Web Services), 5, 147
__getitem__, 96, 102 Azure, 5, 6, 147
__init__, 108, 155 Azure Blob Storage, 144, 145
__len__, 96 Azure Marketplace, 6
__repr__, 64
B
A back translation, 86
AdaGrad, 25 backpropagation through time, 71
Adam, 25, 98 backward() function, 26
AdaptiveAvgPool layer, 38 BadRandom, 130, 164
AdaptiveMaxPool layer, 38 BatchNorm layer, 47, 52, 173
add_graph() function, 119 batch_size, 21
adversarial samples, 177-181 BCEWithLogitsLoss() function, 83
and black box attacks, 180 BertLearner.from_pretrained_model, 184
and defending against adversarial attacks, best_loss, 55
180 Bidirectional Encoder Representations from
AlexNet, 39, 135, 150 Transformers (BERT), 183-185, 189
Amazon Web Services (see AWS) biLSTM (bidirectional LSTM), 73
AMD, 2 Bitcoin, 2
Anaconda, 9 black-box attacks, 180
ApacheMXNet, xii BookCorpus dataset, 183
append_effect_to_chain, 108 Borg, 147
ARG, 142 broadcasting, tensor, 13
argmax() function, 11, 29, 84, 178
C
arXiv, 190
arXiv Sanity Preserver, 190 C++ compiler, 156
attacks C++ library (see libTorch)
adversarial, 180 Caffe2, 159
black-box, 180 CAM (class activation mapping), 122-125
white-box, 180 Candadian Institute for Advanced Research
attention, 181 (CIFAR-10), 115
193

## Page 212

Chainer, xi CrossEntropyLoss() function, 23, 83, 98, 164,
challenges with image classification, 17-21 165
checkbox, 7 CUDA (Compute Unified Device Architec‐
checkpoint_sequential, 136 ture), 8, 132
CIFAR-10 (Candadian Institute for Advanced cuda() function, 27
Research), 115 cuda.is_available() function, 10
CIFAR-10 dataset, 177 custom deep learning machine
class activation mapping, 115, 115 CPU/Motherboard, 2
class activation mapping (CAM), 122-125 GPU, 2
classifier, 52 RAM, 2
cloud platforms, 3-7 storage, 2
Amazon Web Services, 5 custom transform classes, 64
Azure, 6
choosing, 7 D
...
decoder, 168 filesytem, 29
deep learning, defined, x filter, 35
degrees parameter, 62 find_lr() function, 55, 105
deletion, random, 85 fit() function, 184
DenseNet, 43 fit_one_cycle, 188
differential learning rates, 56 flame graphs, 125-132
DigitalOcean, 147 and installing py-spy, 127
discriminator networks, 170, 172 fixing slow transformations, 129-132
distilling, 180 reading, 128
Docker, 175 Flask, 138-140
Docker container, building, 141-143 forward() function, 22, 34, 128
Docker Hub, 141 Fourier transform, 11
download.py script, 18, 20 frequency domain, 99-107
Dropout, x, 52 and frequency masking, 109-110
Dropout layer, 38, 39, 151 and learning rate, 105
and ResNet, 104
E dataset for, 102-104
mel spectrograms, 100-101
embedding matrix, 75
embeddings, for text classification, 74-76
G
encoder, 168
encoding, one-hot, 75 GANs (see generative adversarial networks)
Enhanced Super-Resolution Generative Adver‐ gated recurrent units (GRUs), 73
sarial Network (ESRGAN), 173 gc.collect() function, 134
ENTRYPOINT, 143 GCP (Google Cloud Platform), 5, 6
ENV, 142 GCP Marketplace, 6
Environmental Sound Classification (ESC) generative adversarial networks (GANs),
dataset, 93-98 169-173
building, 96 and ESRGAN, 173
CNN model for, 98 and mode collapse, 172
exploring, 94 neural networks, 170
obtaining, 93 training, 171
playing audio in Jupyter for, 93 generator networks, 170
SoX and LibROSA for, 95 get_stopwords() function, 85
torchaudio, 95 get_synonyms() function, 85
epsilon, 179 GKE (Google Kubernetes Engine), 147, 147
ESC-50, 105 Google, xi
(see also Environmental Sound Classifica‐ Google Cloud Platform (GCP), 5, 6
tion (ESC) dataset) Google Cloud Storage, 144, 145
exploding gradient, 71 Google Colaboratory, 3
EXPOSE, 143 Google Kubernetes Engine (GKE), 147, 147
...
flame graphs, 131 init() function, 22
gradient checkpointing, 134-136 insertion, random, 85
image classification, 27 in_channels, 36
matrix multiplication, 164 in_features, 53
surge, ix item() function, 11, 84
gradient
exploding, 71 J
vanishing, 71
...
I
Karpathy, Andrej, 53
image classification, 15-31 Karpathys constant, 53
activation functions, 22 Keras, xii
and data loaders, 18 Kubernetes (k8s), 141, 147-150
and GPU, 27 cluster creation, 148
building training dataset for, 18-20 scaling services, 149
building validation and test datasets, 20 setting up on GKE, 147
challenges with, 17-21 updates and cleaning up with, 149
creating a network, 22
data for, 17 L
example, 15
...
supervised, 17
Inception, 40, 101, 113, 175
196 | Index

## Page 215

transfer (see transfer learning) mixup, 161-165
unsupervised, 17 MNIST, 115
learning rates MobileNet, 43
and frequency, 105 mode collapse, 172
and ResNet, 53-56 model saving, 29
defined, 25 model serving, 137-146
differential, 56 and local versus cloud storage, 144-145
least recently used (LRU) cache, 103 building a flask service, 138-140
LeNet-5, 39-44 Docker containers, 141-143
LibROSA, 95, 100 logging and telemetry, 145
libTorch, 156-159 setting up model parameters, 140
importing TorchScript model, 157 model.children() function, 121
installation and setup, 156 model.eval() function, 151
Linear layer, 52 models.alexnet(pretrained=True) function, 44
list_effects() function, 108 Motherboard, 2
load() function, 95 MSELoss, 23
load_model() function, 140, 141, 144 multihead attention, 182
load_state_dict() function, 128 MXNet, 159
local storage, cloud versus, 144-145 MySQL, 125
logging, 145
log_spectogram.shape, 101 N
...
M
history, ix
MacBook, 9 loss functions, 23
Mask R-CNN, 175-177 optimizing, 24-26
maskrcnn-benchmark library, 175 recurrent, 69-71
matplotlib, 55, 103 NLP (natural language processing), 69
max() function, 11 nn.Module, 165
MaxPool layer, 39 nn.Sequential layer, 135, 168
MaxPool2d layer, 38 nn.Sequential() function, 34
max_size parameter, 80 nn.Sequential() layer, 121
max_width parameter, 110 NumPy, 13
md5sum, 9 NVIDIA GeForce RTX 2080 Ti, 2, 132
mean function, 66 Nvidia GTX 1080 Ti, 2, 4
mean, plotting, 121 Nvidia RTX 2080 Ti, 2, 3
mel scale, 100 nvidia-smi, 134
mel spectrograms, 100-101
Microsoft Azure (see Azure) O
MIcrosoft Cognitive Toolkit, 159
object detection, 173-175
Index | 197

## Page 216

OK, 139 setting up model parameters, 140
one-hot encoding, 75 TorchScript, 150-156
ones() function, 10 py-spy, 127, 130
ONNX (Open Neural Network Exchange), 159 Python, 121, 137
OpenAI, 181, 185 Python 2.x, 8
optim.Adam() function, 26 PyTorch (generally), 1-14
optimization of neural networks, 24-26 building a custom deep learning machine,
optimizer.step() function, 26 1-3
out_channels, 36 cloud platforms and, 3-7
out_features, 53 installation, 8-10
overfitting, 20, 57 origins, xi
tensors and, 10-13
P PyTorch Hub, 48
...
R
p2.xlarge, 5
pad token, 81 Raina, Rajat, x
PadTrim, 107 RAM, 2
pandas, 77 random deletion, 85
parameters() function, 52 random insertion, 85
partial() function, 121 random swap, 86
PCPartPicker, 3 RandomAffine, 62
permute() function, 13 RandomApply, 64
pip, 95, 138 RandomCrop, 60
plt function, 55 RandomGrayscale, 59
pod, 148 RandomResizeCrop, 60
pooling in CNN, 37 RBG color space, 63
predict() function, 140 README, 93
predictions, 176 rectified linear unit (see ReLU)
and ensembling, 66 recurrent neural networks (RNNs), 69-71, 181
in image classification, 28 Red Hat Enterprise Linux (RHEL) 7, 9
with torchtext, 84 register_backward_hook() function, 120
preprocess() function, 84 ReLU (rectified linear unit), 22, 30, 39, 52
pretrained models, 44-48 remove() function, 120
BatchNorm, 47 requires_grad() function, 52
choosing, 48 resample, 63
examining model structure, 44-47 reshape() function, 12
print() function, 120, 151 reshaping a tensor, 12
print(model) function, 47 Resize(64) transform, 19
process() function, 84 ResNet architecture, 43, 48, 101, 175
production, deploying PyTorch applications in, and frequency, 104
137-160 and learning rate, 53-56
building a flask service, 138-140 transfer learning with, 51-53
deploying on Kubernetes, 147-150 ResNet-152, 132
Docker containers, 141-143 ResNet-18, 120
libTorch, 156-159 RHEL (Red Hat Enterprise Linux) 7, 9
local versus cloud storage, 144-145 RMSProp, 25
logging and telemetry, 145 RNNs (recurrent neural networks), 69-71, 181
model serving, 137-146 ROCm, 2
198 | Index

## Page 217

RUN, 142 in custom deep learning machine, 2
run_gpt2.py, 187 local versus cloud, 144-145
SummaryWriter, 117
S super-resolution, 166-173
and GANs, 169-173
Salesforce, xi
...
T
SimpleNet, 30
simplenet.parameters() function, 26 telemetry, 145
slow transformations, fixing, 129-132 tensor processing units (TPUs), x, 6
Smith, Leslie, 53 tensor.mean() function, 110
softmax function, 22 TensorBoard, 116-125
softmax() function, 23 and PyTorch hooks, 120
sound, 91-113 class activation mapping, 122-125
about, 91 installing, 116
and ESC-50 dataset, 93-98 plotting mean and standard deviation with,
audio data augmentation, 107-113 121
frequency domain, 99-107 sending data to, 117-120
frequency masking, 109-110 TensorFlow, xi, 150, 185
in Jupyter Notebook, 93 tensors, 11-13
mel spectrograms, 100-101 broadcasting, 13
SoX effect chains, 107 operations, 11-13
SpecAugment, 108-113 TeslaV100, 3, 6
time masking, 111-113 test datasets, building, 20
torchaudio, 95 text classification, 69-88
torchaudio transforms, 107 and transfer learning, 88
SoX, 95 back translation, 86
SoX effect chains, 107 biLSTM, 73
sox_build_flow_effects(), 108 data augmentation, 84-88
spaCy, 77 embeddings for, 74-76
SpecAugment, 108-113 gated recurrent units, 73
frequency masking, 109-110 in Long Short-Term Memory Networks,
time masking, 111-113 71-72
squeeze(0) function, 29 random deletion, 85
SqueezeNet, 43 random insertion, 85
SSH (Secure Shell), 4 random swap, 86
stacktrace, 125, 126 recurrent neural networks, 69-71
standard deviation, plotting, 121 torchtext, 76-84
startup, 142, 144 text generation, with GPT-2, 185-187
state_dict() function, 140 tf.keras, xii
stochastic gradient descent (SGD), 25 Theano, xii
storage time step, 70
Index | 199

## Page 218

to() function, 11, 27 torchvision transforms, 58-63
top-5, 39, 43 with ResNet, 51-53
torch.argmax() function, 140 transformations, fixing slow, 129-132
torch.distribution.Beta, 164 Transformer architecture, 181-189
torch.hub.list(pytorch/vision) function, 48 attention, 181
torch.jit.save, 153, 154 BERT, 183-185
torch.jit.save() function, 156, 157 choosing, 189
torch.load() function, 29 FastBERT, 183-185
torch.nn.ConvTranspose2d layer, 168 GPT-2, 185-187
torch.save() function, 29, 140 multihead attention, 182
torch.topk() function, 123 ULMFiT, 187-189
torch.utils.checkpoint_sequential() function, transforms.ToTensor() function, 19
135 TWEET, 79, 84
torch.utils.tensorboard, 117 Twitter, 77
torchaudio, 95, 107
torchaudio transforms, 107 U
torchaudio.load(), 108
...
W
torchvision, 18
torchvision transforms, 58-63 Waitress (web server), 143
torchvision.models, 48 waveform, 91, 100, 107
TPUs (tensor processing units), x, 6 web, 5
tracing, 150-153 white-box attack, 180
train() function, 28 WikiText-103 dataset, 187
train_net.py, 177 word2vec, 76
transfer learning, 51-67
and data augmentation, 57-66 X
