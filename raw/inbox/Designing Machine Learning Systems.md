...
Complex because they consist of many different components “This is, simply, the
and involve many different stakeholders. Unique because very best book you
they’re data dependent, with data varying wildly from one can read about how
...
—Josh Wills
design decision—such as how to process and create training Software Engineer at WeaveGrid and
data, which features to use, how often to retrain models, former Director of Data Engineering, Slack
...
—Jacopo Tagliabue
evaluating, deploying, and updating models Director of AI, Coveo
...
• Architecting an ML platform that serves across use cases a platform for real-time machine learning.
...
Designing Machine Learning Systems is a welcome addition to the field of applied machine
...
Google, and Coauthor of Reliable Machine Learning
...
approximation of the exact output, but often, it’s good enough. You can see a lot
...
from similar distributions. You might ask: “If the data is unseen, how do we
...
Due to the way most ML algorithms today learn, ML solutions will especially shine if
...
run in your photo editing app to suggest how best to enhance your photos. You might
...
customers perceive your brand. You might want to know when/where/how it’s men‐
...
Research Production
Requirements State-of-the-art model performance on Different stakeholders have different
...
14 It’s not unusual for the ML and data science teams to be among the first to go during a company’s mass
...
31 Xinyun Chen, Chang Liu, Bo Li, Kimberly Lu, and Dawn Song, “Targeted Backdoor Attacks on Deep
...
requirements.
You might wonder: with all these objectives, requirements, and processes in place,
...
1 Eugene Yan has a great post on how data scientists can understand the business intent and context of the
...
prediction.3 You might reason that online prediction will enable recommendations
...
Yet, even rigorous experiments might not be sufficient to understand the relationship
...
6 Parmy Olson, “Nearly Half of All ‘AI Startups’ Are Cashing In on Hype,” Forbes, March 4, 2019,
...
Figure 2-1. How long it takes for a company to bring a model to production is propor‐
...
8 Up-scaling and down-scaling are two aspects of “scaling out,” which is different from “scaling up.” Scaling out
...
However, handling growth isn’t just resource scaling, but also artifact management.
...
models, both the monitoring and retraining aspect will need to be automated. You’ll
...
11.. Choose a metric to optimize. For example, you might want to optimize for
impressions—the number of times an ad is shown.
22.. Collect data and obtain labels.
33.. Engineer features.
44.. Train models.
55.. During error analysis, you realize that errors are caused by the wrong labels, so
you relabel the data.
66.. Train the model again.
77.. During error analysis, you realize that your model always predicts that an ad
...
88.. Train the model again.
99.. The model performs well on your existing test data, which is by now two months
...
1100.. Train the model again.
1111.. Deploy the model.
1122.. The model seems to be performing well, but then the businesspeople come
...
Later chapters will dive deeper into what each of these steps requires in practice.
...
12 Note that objective functions are mathematical functions, which are different from the business and ML
...
SFist, September 24, 2019, https://oreil.ly/PXtGi; Kevin Roose, “The Making of a YouTube Radical,” New York
...
“machine learning is inherently a multiobjective task” (Yaochu Jin and Bernhard Sendhoff, “Pareto-Based
...
16 Anand Rajaraman, “More Data Usually Beats Better Algorithms,” Datawocky, March 24, 2008,
...
1 “Interesting” in production usually means catastrophic, such as a crash or when your cloud bill hits an
astronomical amount.
...
4 John Koetsier, “Apple Just Crippled IDFA, Sending an $80 Billion Industry Into Upheaval,” Forbes, June 24,
2020, https://oreil.ly/rqPX9.
5 Patrick McGee and Yuan Yang, “TikTok Wants to Keep Tracking iPhone Users with State-Backed Work‐
...
Imagine we have a dataset of 1,000 examples, and each example has 10 features. If
...
formats.”8
8 “Announcing Amazon Redshift Data Lake Export: Share Data in Apache Parquet Format,” Amazon AWS,
December 3, 2019, https://oreil.ly/ilDb6.
Data Formats | 57

## Page 78

Figure 3-3. When stored in CSV format, my interview file is 14 MB. But when stored in
...
in Figure 3-4. Relations are unordered. You can shuffle the order of the rows or the
...
12 Greg Kemnitz, a coauthor of the original Postgres paper, shared on Quora that he once wrote a reporting SQL
...
15 Matthias Boehm, Alexandre V. Evfimievski, Niketan Pansare, and Berthold Reinwald, “Declarative Machine
...
16 James Phillips, “Surprises in Our NoSQL Adoption Survey,” Couchbase, December 16, 2014,
...
In the document model, all information about a book can be stored in a document,
...
However, there is no guarantee that all lines must follow this format. You can add a
...
storage doesn’t follow a schema, you can store any type of data. You can convert all
...
ride through a ride-sharing service, uploading a new model, watching a YouTube
...
19 This paragraph, as well as many parts of this chapter, is inspired by Martin Kleppmann’s Designing Data-
...
Because each transaction is often processed as a unit separately from other transac‐
...
You might want to join data from multiple sources and clean it. You might want to
standardize the value ranges (e.g., one data source might use “Male” and “Female” for
genders, but another uses “M” and “F” or “1” and “2”). You can apply operations such
as transposing, deduplicating, sorting, aggregating, deriving new features, more data
validating, etc.
21 Tino Tereshko, “Separation of Storage and Compute in BigQuery,” Google Cloud blog, November 29,
...
from Compute in Your Data Lake with Object Storage,” IBM blog, September 21, 2017, https://oreil.ly/
...
23 In the first draft of this book, I had cost as a reason why you shouldn’t store everything. However, as of today,
storage has become so cheap that the storage cost is rarely a problem.
Data Storage Engines and Processing | 71

## Page 92

and infrastructures become standardized, data structures also become standardized.
...
24 In practice, the price optimization might not have to request the predicted number of rides/drivers every time
...
Figure 3-8. In the request-driven architecture, each service needs to send requests to two
other services
...
Figure 3-9. With a broker, a service only has to communicate with the broker instead of
with other services
...
from Stackshare
27 If you want to learn more about how Apache Kafka works, Mitch Seymour has a great animation to explain it
using otters!
...
You need infrastructure that allows you to process streaming data as well as batch
...
Quota sampling
You select samples based on quotas for certain slices of data without any random‐
...
of sampling methods. You’re right. Unfortunately, in many cases, the selection of data
...
2 James J. Heckman, “Sample Selection Bias as a Specification Error,” Econometrica 47, no. 1 (January 1979):
...
3 Rachel Lerman, “Google Is Testing Its Self-Driving Car in Kirkland,” Seattle Times, February 3, 2016,
...
number, k, of tweets to do analysis or train a model on. You don’t know how many
tweets there are, but you know you can’t fit them all in memory, which means you
don’t know in advance the probability at which a tweet should be selected. You want
to ensure that:
•• Every tweet has an equal probability of being selected.
•• You can stop the algorithm at any time and the tweets are sampled with the
correct probability.
...
11.. Put the first k elements into the reservoir.
22.. For each incoming nth element, generate a random number i such that 1 ≤ i ≤ n.
33.. If 1 ≤ i ≤ k: replace the ith element in the reservoir with the nth element. Else, do
nothing.
...
k
This means that each incoming nth element has probability of being in the reser‐
n
k
voir. You can also prove that each element in the reservoir has probability of
n
...
P x
. Q(x) is called the proposal distribution or the importance distribution. Q(x) can
Q x
...
Q x
P x P x
E P x x = ∑P x x= ∑Q x x Q x =E Q x x Q x
x x
One example where importance sampling is used in ML is policy-based reinforce‐
ment learning. Consider the case when you want to update your policy. You want to
...
7 Xiaojin Zhu, “Semi-Supervised Learning with Graphs” (doctoral diss., Carnegie Mellon University, 2005),
https://oreil.ly/VYy4C.
88 | Chapter 4: Training Data

## Page 109

Slow labeling leads to slow iteration speed and makes your model less adaptive to
...
Consider this simple task of entity recognition. You give three annotators the follow‐
...
You receive back three different solutions, as shown in Table 4-1. Three annotators
...
samples. Your ML engineers are confident that more data will improve the model
...
Natural Labels
Hand-labeling isn’t the only source for labels. You might be lucky enough to work
...
However, not all recommender systems have minute-long feedback loops. If you
work with longer content types like blog posts or articles or YouTube videos, the
...
learning another task for your new task Yes for fine-tuning, though the number of ground truths
...
Steven Yoo, and Wenzhe Shi, “Addressing Delayed Feedback for Continuous Training with Neural Networks
...
11 Alexander Ratner, Stephen H. Bach, Henry Ehrenberg, Jason Fries, Sen Wu, and Christopher Ré, “Snorkel:
...
ments. You only need to see a small, cleared subset of data to write LFs, which can be
...
13 Jared A. Dunnmon, Alexander J. Ratner, Khaled Saab, Matthew P. Lungren, Daniel L. Rubin, and Christopher
...
A classic semi-supervision method is self-training. You start by training a model on
...
of classifying the topic of Twitter hashtags. You can start by labeling the hashtag
...
16 Avrim Blum and Tom Mitchell, “Combining Labeled and Unlabeled Data with Co-Training,” in Proceedings of
...
token.
17 Avital Oliver, Augustus Odena, Colin Raffel, Ekin D. Cubuk, and Ian J. Goodfellow, “Realistic Evaluation of
...
19 Jeremy Howard and Sebastian Ruder, “Universal Language Model Fine-tuning for Text Classification,” arXiv,
...
method.23 You need a committee of several candidate models, which are usually the
...
next, and it might vote based on how uncertain it is about the prediction. You then
...
26 Andrew Ng, “Bridging AI’s Proof-of-Concept to Production Gap” (HAI Seminar, September 22, 2020), video,
...
want to create training data to detect whether an email is spam or not. You decide
...
goftaar 2019).
32 Nathalie Japkowciz and Shaju Stephen, “The Class Imbalance Problem: A Systematic Study,” 2002,
...
Many classification problems can be modeled as regression problems. Your model
...
35 As of July 2021, when you use scikit-learn.metrics.f1_score, pos_label is set to 1 by default, but you can
...
36 Jesse Davis and Mark Goadrich, “The Relationship Between Precision-Recall and ROC Curves,” Proceedings of
...
One such technique is two-phase learning.42 You first train your model on the resam‐
pled data. This resampled data can be achieved by randomly undersampling large
classes until each class has only N instances. You then fine-tune your model on the
original data.
...
that if there are two instances, x and x , and the loss resulting from making the
1 2
wrong prediction on x is higher than x , the model will prioritize making the correct
1 2
prediction on x over making the correct prediction on x . By giving the training
1 2
...
The method started by using a cost matrix to specify C : the cost if class i is classified
ij
...
as the other way around, you can make C twice as high as C .
10 01
...
Predicted NEGATIVE C(0, 0) = C C(1, 0) = C
00 10
Predicted POSITIVE C(0, 1) = C C(1, 1) = C
01 11
...
47 Mikel Galar, Alberto Fernandez, Edurne Barrenechea, Humberto Bustince, and Francisco Herrera, “A Review
...
an image while preserving its label. You can modify the image by cropping, flipping,
...
48 Alex Krizhevsky, Ilya Sutskever, and Geoffrey E. Hinton, “ImageNet Classification with Deep Convolutional
...
51 Ian J. Goodfellow, Jonathon Shlens, and Christian Szegedy, “Explaining and Harnessing Adversarial Exam‐
ples,” arXiv, March 20, 2015, https://oreil.ly/9v2No; Ian J. Goodfellow, David Warde-Farley, Mehdi Mirza,
Aaron Courville, and Yoshua Bengio, “Maxout Networks, arXiv, February 18, 2013, https://oreil.ly/L8mch.
...
as 1). From example x of label DOG and example x of label CAT, you can generate x'
1 2
such as:
x′=γx + 1−γ x
1 2
The label of x' is a combination of the labels of x and x : γ×0+ 1−γ ×1. This
1 2
...
55 Hongyi Zhang, Moustapha Cisse, Yann N. Dauphin, and David Lopez-Paz, “mixup: Beyond Empirical Risk
Minimization,” ICLR 2018, https://oreil.ly/lIM5E.
56 Veit Sandfort, Ke Yan, Perry J. Pickhardt, and Ronald M. Summers, “Data Augmentation Using Generative
...
1 Loris Nanni, Stefano Ghidoni, and Sheryl Brahnam, “Handcrafted vs. Non-handcrafted Features for Com‐
...
Figure 5-1. An example of techniques that you can use to handcraft n-gram features for
your text
...
Feature engineering requires knowledge of domain-specific techniques—in this case,
...
Your model will hopefully learn to extract useful features from this. In this new
...
2 In my experience, how well a person handles missing values for a given dataset during interviews strongly
...
less than 0.1%. You don’t want to do row deletion if that means 10% of your data
samples are removed.
...
3 Rachel Bogardus Drew, “3 Facts About Marriage and Homeownership,” Joint Center for Housing Studies of
...
x′= x− min x
max x − min x
You can validate that if x is the maximum value, the scaled value x′ will be 1. If x is the
...
original data.5
5 Changyong Feng, Hongyue Wang, Naiji Lu, Tian Chen, Hua He, Ying Lu, and Xin M. Tu, “Log-
...
Figure 5-3. In many cases, the log transformation can help reduce the skewness of
your data
...
that easy. You can try to plot the histograms of the values and choose the boundaries
...
categories is straightforward. You can just give each category a number and you’re
done.
...
The number of brands is overwhelming, but you think: “I can still handle this.” You
encode each brand as a number, so now you have two million numbers, from 0 to
1,999,999, corresponding to two million brands. Your model does spectacularly on
...
Your model doesn’t crash anymore, but your sellers complain that their new
...
UNKNOWN brand. You fix this by encoding only the top 99% most popular brands
and encode the bottom 1% brand as UNKNOWN. This way, at least your model
knows how to deal with UNKNOWN brands.
Your model seems to work fine for about one hour, then the click-through rate on
...
Finding a way to solve this problem turns out to be surprisingly difficult. You don’t
...
You can choose a hash space large enough to reduce the collision. You can also
...
8 Lucas Bernardi, “Don’t Be Tricked by the Hashing Trick,” Booking.com, January 10, 2018,
...
these two features will result in a feature with 100 × 100 = 10,000 possible values. You
...
You Need” (Vaswani et al. 2017), positional embedding has become a standard data
...
If we rescale the positions to between 0 and 1, so 0, 1, 2, …, 7 become 0, 0.143, 0.286,
...
Figure 5-6. Example of fixed position embedding. H is the dimension of the outputs
...
al. 2020).
T T T T T
γ v = a 1 cos 2πb 1 v ,a 1 sin 2πb 1 v ,...,a mcos 2πb m v ,a msin 2πb m v
Data Leakage
...
which outputs slightly different CT scan images. Your model learned to rely on
...
12 Will Douglas Heaven, “Hundreds of AI Tools Have Been Built to Catch Covid. None of Them Helped,” MIT
...
Figure 5-7. Split data by time to prevent future information from leaking into the
training process
...
data was generated.
15 Björn Barz and Joachim Denzler, “Do We Train on Test Data? Purging CIFAR of Near-Duplicates,” Journal of
...
Do ablation studies to measure how important a feature or a set of features is to your
...
You can store removed features to add them back later. You can also just store general
...
19 Scott Lundberg, SHAP (SHapley Additive exPlanations), GitHub repository, last accessed 2021,
...
Measuring feature generalization is a lot less scientific than measuring feature impor‐
...
splits don’t come from the same distribution. You might want to investigate whether
...
will take for a given taxi ride. You retrain this model every week, and you want to
...
On the other hand, HOUR_OF_THE_DAY is a great feature, because the time in the
...
tion and specificity. You might realize that the traffic during an hour only changes
...
•• Understand how your data is generated, collected, and processed. Involve
...
concepts might find helpful the section “Basic ML Reviews” in the book’s GitHub
repository.
...
(like BERT or GPT-3) might be used to generate embeddings to input into a logistic
regression model.
...
BERT model outperforms the best gradient-boosted tree by 5%. Your team decides to
...
a couple of experiments for the architecture you’re evaluating it against. You might
...
1 Andrew Ng has a great lecture where he explains that if a learning algorithm suffers from high bias, getting
...
training data, but it can give you a sense of whether you can expect any performance
...
2 I went through the winning solutions listed on Farid Rashidi’s “Kaggle Solutions” web page. One solu‐
...
3 Mikel Galar, Alberto Fernandez, Edurne Barrenechea, Humberto Bustince, and Francisco Herrera, “A Review
on Ensembles for the Class Imbalance Problem: Bagging-, Boosting-, and Hybrid-Based Approaches,” IEEE
...
11.. You start by training the first weak classifier on the original dataset.
22.. Samples are reweighted based on how well the first classifier classifies them, e.g.,
misclassified samples are given higher weight.
33.. Train the second classifier on this reweighted dataset. Your ensemble now con‐
sists of the first and the second classifiers.
44.. Samples are weighted based on how well the ensemble classifies them.
55.. Train the third classifier on this reweighted dataset. Add the third classifier to the
ensemble.
66.. Repeat for as many iterations as needed.
77.. Form the final strong classifier as a weighted combination of the existing classifi‐
...
Versioning
Imagine this scenario. You and your team spent the last few weeks tweaking your
model, and one of the runs finally showed promising results. You wanted to use it
...
You remembered that you’d made some changes to the code between that run and the
...
This problem could have been avoided if you versioned your ML experiments. ML
...
10 Notable examples include atomic operations in CUDA where nondeterministic orders of operations lead to
...
Debugging should be both preventive and curative. You should have healthy practices
...
There is, unfortunately, still no scientific approach to debugging in ML. However,
...
11 For products that serve a large number of users, you also have to care about scalability in serving a model,
...
13 Tim Salimans, Yaroslav Bulatov, and contributors, gradient-checkpointing repository, 2017,
...
18 Feng Niu, Benjamin Recht, Christopher Ré, and Stephen J. Wright, “Hogwild!: A Lock-Free Approach to
...
number of hidden layers, number of hidden units, dropout probability, β and β in
1 2
...
24 GSD is a well-documented technique. See “How Do People Come Up With All These Crazy Deep Learning
...
28 Barret Zoph and Quoc V. Le, “Neural Architecture Search with Reinforcement Learning,” arXiv, November
...
29 You can make the search space continuous to allow differentiation, but the resulting architecture has to be
...
Since learned optimizers are neural networks, they need to be trained. You can train
...
31 Luke Metz, Niru Maheswaranathan, C. Daniel Freeman, Ben Poole, and Jascha Sohl-Dickstein, “Tasks,
...
of data, start with non-ML solutions. Your first stab at the problem can be the
...
33 Samantha Murphy, “The Evolution of Facebook News Feed,” Mashable, March 12, 2013,
...
you a 100% boost, then a heuristic will get you 50% of the way there.”35 You might
...
You’ll also want to experiment to figure out how quickly your model decays in
...
ML. You might want to partner with the business team to develop metrics for model
...
38 Fréchet inception distance, a common metric for measuring the quality of synthesized images. The smaller
...
Evaluation metrics, by themselves, mean little. When evaluating your model, it’s
...
you might never be able to convince your users to trust this system. Even if your
...
It’s crucial to compare your new model to these existing solutions. Your ML
...
clips to simulate the variance in your users’ recordings. You might want to choose the
...
41 Other examples of noisy data include images with different lighting or texts with accidental typos or inten‐
...
because A is much more likely to watch romance than any other type of movies. You
...
ad A and ad B. Your model predicts that this user will click on ad A with a 10%
probability and on ad B with an 8% probability. You don’t need your model to be
...
44 For more information on calibrated recommendations, check out the paper “Calibrated Recommendations”
by Harald Steck in 2018 based on his work at Netflix.
Model Offline Evaluation | 183

## Page 204

5% of the time, your estimated number of clicks will be way off. If you have another
...
your critical slices are. You might wonder how to discover critical slices in your data.
...
46 P. J. Bickel, E. A. Hammel, and J. W. O’Connell, “Sex Bias in Graduate Admissions: Data from Berkeley,”
...
We then continued to cover different aspects of model development. We covered not
...
opment environment. Your model can be deployed to a staging environment for
...
You can use this exposed endpoint for downstream applications: e.g., when an appli‐
...
If you come from an academic background, some of the topics discussed in this
...
You might argue that, if so, only a small number of companies need to worry
...
A simplified architecture for online prediction that uses both streaming features and
...
panies when first deploying a model. You export your model, upload the exported
...
15 David Curry, “Grubhub Revenue and Usage Statistics (2022),” Business of Apps, January 11, 2022,
...
Chapter 10.
18 Shuyi Chean and Fabian Hueske, “Streaming SQL to Unify Batch & Stream Processing w/ Apache Flink
...
19 Yu Cheng, Duo Wang, Pan Zhou, and Tao Zhang, “A Survey of Model Compression and Acceleration for
...
21 Forrest N. Iandola, Song Han, Matthew W. Moskewicz, Khalid Ashraf, William J. Dally, and Kurt Keutzer,
...
23 Geoffrey Hinton, Oriol Vinyals, and Jeff Dean, “Distilling the Knowledge in a Neural Network,” arXiv, March
...
26 Jonathan Frankle and Michael Carbin, “The Lottery Ticket Hypothesis: Finding Sparse, Trainable Neural
...
30 Matthieu Courbariaux, Yoshua Bengio, and Jean-Pierre David, “BinaryConnect: Training Deep Neural Net‐
...
x nanoseconds, it’ll take 32x nanoseconds for 32-bit numbers but only 16x nanosec‐
...
32 As of October 2020, TensorFlow’s quantization aware training doesn’t actually train models with weights in
...
35 Itay Hubara, Matthieu Courbariaux, Daniel Soudry, Ran El-Yaniv, and Yoshua Bengio, “Quantized Neural
...
36 Quoc Le and Kip Kaehler, “How We Scaled Bert To Serve 1+ Billion Daily Requests on CPUs,” Roblox, May
...
connections.
37 Amir Efrati and Kevin McLaughlin, “As AWS Use Soars, Companies Surprised by Cloud Bills,” The Infor‐
...
This vision comes together in the AI Engineering team, where our expertise is used to
...
46 For a helpful visualization of loop tiling, see slide 33 from Colfax Research’s presentation “Access to Caches
...
48 Shashank Prasanna, Prethvi Kashinkunti, and Fausto Milletari, “TensorRT 3: Faster TensorFlow Inference and
...
ML researcher, come up with a new model architecture? You might need to optimize
...
49 This is also why you shouldn’t read too much into benchmarking results, such as MLPerf’s results. A popular
...
If you use PyTorch on GPUs, you might have seen torch.backends.cudnn.bench
...
11.. It first breaks your computation graph into subgraphs.
22.. It predicts how big each subgraph is.
33.. It allocates time to search for the best possible path for each subgraph.
44.. It stitches the best possible way to run each subgraph together to execute the
entire graph.
...
they can be slow. You go through all the possible paths and find the most optimized
...
sessions. You optimize your model once for one hardware backend then run it on
...
WASM is one of the most exciting technological trends I’ve seen in the last couple of
...
The main drawback of WASM is that because WASM runs in browsers, it’s slow.
Even though WASM is already much faster than JavaScript, it’s still slow compared to
running code natively on devices (such as iOS or Android apps). A study by Jangda et
al. showed that applications compiled to WASM run slower than native applications
...
Figure 7-16. As hardware becomes more powerful, ML models will move to online and
on the edge
...
1 This seems to be a fairly common pattern for inventory prediction. Eugene Yan wrote about a similar story to
...
His company learned the hard way an important lesson that the rest of the industry
...
2 This is one of the reasons why many companies are hesitant to use products by startups, and why many
...
6 Back when in-person academic conferences were still a thing, I often heard researchers arguing about whose
...
8 John Mcquaid, “Limits to Growth: Can AI’s Voracious Appetite for Data Be Tamed?” Undark, October 18,
...
10 This means the self-driving car is a bit safer than an average human driver. As of 2019, the ratio of
...
predicts whether someone is qualified, so it recommends resumes with feature X. You
...
13 Ray Jiang, Silvia Chiappa, Tor Lattimore, András György, and Pushmeet Kohli, “Degenerate Feedback Loops
...
feedback loops.
15 Erik Brynjolfsson, Yu (Jeffrey) Hu, and Duncan Simester, “Goodbye Pareto Principle, Hello Long Tail:
...
18 Gediminas Adomavicius and YoungOk Kwon, “Improving Aggregate Recommendation Diversity Using
...
If the position in which a prediction is shown affects its feedback in any way, you
...
•• P(X, Y) = P(Y|X)P(X)
•• P(X, Y) = P(X|Y)P(Y)
20 Jeffrey C. Schlimmer and Richard H. Granger, Jr., “Incremental Learning from Noisy Data,” Machine Learning
...
To make this concrete, consider the task of detecting breast cancer. You know that the
risk of breast cancer is higher for women over the age of 40,23 so you have a variable
“age” as your input. You might have more women over the age of 40 in your training
...
21 You might wonder what about the case when P(X|Y) changes but P(Y) remains the same, as in the second
...
is a feature. Your company’s marketing department recently launched a campaign that
...
24 Arthur Gretton, Alex Smola, Jiayuan Huang, Marcel Schmittfull, Karsten Borgwardt, and Bernard
...
P(Y) changes but P(X|Y) remains the same. You can think of this as the case when the
...
27 Han Zhao, Remi Tachet Des Combes, Kun Zhang, and Geoffrey Gordon, “On Learning Invariant Rep‐
...
the same but the conditional distribution of the output given an input changes. You
...
performance.
29 If you use a neural network using softmax as your last layer for your classification tax, the dimension of this
...
30 You don’t need ground truth labels if you use an unsupervised learning method, but the vast majority of
...
31 Hamel Husain gave a great lecture on why TensorFlow Extended’s skew detection is so bad for CS 329S:
Machine Learning Systems Design (Stanford, 2022). You can find the video on YouTube.
...
32 I. M. Chakravarti, R. G. Laha, and J. Roy, Handbook of Methods of Applied Statistics, vol. 1, Techniques of
...
37 Manuel Baena-García, José del Campo-Ávila, Raúl Fidalgo, Albert Bifet, Ricard Gavaldà, and Rafael Morales-
...
industry.44
41 As Han-chung Lee, one early reviewer, pointed out, this is also because smaller companies don’t have enough
...
45 Some monitoring vendors claim that their solutions are able to detect not only when your model should be
...
46 “Amazon Compute Service Level Agreement,” Amazon Web Services, last updated August 24, 2021,
...
47 Be careful when using the completion rate as a metric to optimize for, as it might bias your recommender
system toward short videos.
252 | Chapter 8: Data Distribution Shifts and Monitoring

## Page 273

predictions are easy to visualize, and their summary statistics are straightforward to
compute and interpret.
You can monitor predictions for distribution shifts. Because predictions are low
...
input distribution.
You can also monitor predictions for anything odd happening, such as predicting an
...
Figure 8-7. Some of the built-in feature validation functions by Great Expectations and
...
49 Ian Malpass, “Measure Anything, Measure Everything,” Code as Craft, February 15, 2011,
...
problem.53 You can use real-time transport such as Kafka or Amazon Kinesis to
...
on their own. You still need experience and statistical knowledge. Consider the two
...
analyzed.
54 Suman Karumuri, Franco Solleza, Stan Zdonik, and Nesime Tatbul, “Towards Observability Data Manage‐
...
1 Joan Serrà, Dídac Surís, Marius Miron, and Alexandros Karatzoglou, “Overcoming Catastrophic Forgetting
...
2 It’s “stateful training” instead of “stateful retraining” because there’s no re-training here. The model continues
...
training, the training frequency is just a knob to twist. You can update your models
...
You might wonder: stateful training sounds cool, but how does this work if I want to
...
4 Mu Li, Li Zhou, Zichao Yang, Aaron Li, Fei Xia, David G. Andersen, and Alexander Smola, “Parameter Server
...
their recommender system to each user within minutes. You download the app and,
...
that at 10:33 p.m., user A clicks on the product with the ID of 32345. Your system
...
You can update the neural network model with a data batch of any size. You can even
...
17 James Vincent, “Twitter Taught Microsoft’s Friendly AI Chatbot to Be a Racist Asshole in Less Than a Day,”
...
11.. A model to detect fraudulent transactions
22.. A model to recommend relevant products to users
33.. A model to predict whether a seller is abusing a system
44.. A model to predict how long it will take to ship an order
Because your team is focusing on developing new models, updating existing models
takes a backseat. You update an existing model only when the following two condi‐
...
obvious problems. You have anywhere between 5 and 10 models in production.
Your priority is no longer to develop new models, but to maintain and improve
...
previous stage has grown into a pain point too big to be ignored. Your team decides
...
11.. Pull data.
22.. Downsample or upsample this data if necessary.
33.. Extract features.
44.. Process and/or annotate labels to create training data.
55.. Kick off the training process.
66.. Evaluate the newly trained model.
77.. Deploy it.
...
A scheduler is basically a tool that handles task scheduling, which we’ll cover in the
...
You might need a more mature model store like Amazon SageMaker (managed
...
frequency. You read the section “Stateless Retraining Versus Stateful Training” on
...
So in this stage, you reconfigure your automatic updating script so that, when the
...
3.32, model version 2.11, model version 1.64. You might want to know how these
...
Requirements. The move from stage 3 to stage 4 is steep. You’ll first need a mecha‐
...
You’ll also need a solid pipeline to continually evaluate your model updates. Writing
...
25 Xinran He, Junfeng Pan, Ou Jin, Tianbing Xu, Bo Liu, Tao Xu, Tanxin Shi, et al., “Practical Lessons from
Predicting Clicks on Ads at Facebook,” in ADKDD ’14: Proceedings of the Eighth International Workshop on
...
architecture and features but you refresh this model with new data). You might
...
model on your specific task. You’ll have to do experiments to find out.
...
The first type of model evaluation you might think about is the good old test splits
...
11.. Deploy the candidate model in parallel with the existing model.
22.. For each incoming request, route it to both models to make predictions, but only
serve the existing model’s prediction to the user.
33.. Log the predictions from the new model for analysis purposes.
...
11.. Deploy the candidate model alongside the existing model.
22.. A percentage of traffic is routed to the new model for predictions; the rest
...
33.. Monitor and analyze the predictions and user feedback, if any, from both models
...
27 Ron Kohavi and Stefan Thomke, “The Surprising Power of Online Experiments,” Harvard Business Review,
September–October 2017, https://oreil.ly/OHfj0.
Test in Production | 283

## Page 304

better accuracy than model B, we can’t tell whether it’s because A is better than B or
...
11.. Deploy the candidate model alongside the existing model. The candidate model
is called the canary.
22.. A portion of the traffic is routed to the candidate model.
33.. If its performance is satisfactory, increase the traffic to the candidate model. If
not, abort the canary and route all the traffic back to the existing model.
44.. Stop when either the canary serves all the traffic (the candidate model has
...
bandit, hence the name. You don’t know which slot machine gives the highest payout.
You can experiment over time to find out which slot machine is the best while maxi‐
...
31 Olivier Chapelle, Thorsten Joachims, Filip Radlinski, and Yisong Yue, “Large-Scale Validation and Analysis
...
33 Greg Rafferty, “A/B Testing—Is There a Better Way? An Exploration of Multi-Armed Bandits,” Towards Data
...
feedback problem, also known as bandit feedback. You can also think of contextual
...
user, or because it’s been shown but not clicked on. You want to show users the items
...
36 Lihong Li, Wei Chu, John Langford, and Robert E. Schapire, “A Contextual-Bandit Approach to Personalized
...
that the results might be variable. One data scientist might perform a set of tests and
...
day.6
1 Kunal Shah, “This Is What Makes SEO Important for Every Business,” Entrepreneur India, May 11, 2020,
...
Tesla AI Day 2021 on YouTube.
3 The definition for “reasonable scale” was inspired by Jacopo Tagliabue in his paper “You Do Not Need a
...
or spread out over multiple locations.8 Your storage layer can be on-prem in a private
...
resources available determines the scalability of your workloads. You can think of the
...
8 I’ve seen a company whose data is spread over Amazon Redshift and GCP BigQuery, and their engineers are
...
number of float point operations a compute unit can run per second. You might
...
10 As of writing this book, an ML workload typically requires between 4 GB and 8 GB of memory; 16 GB of
...
You can see the number of cores and memory offered by some AWS EC2 and GCP
...
13 For readers interested in FLOPS and bandwidth and how to optimize them for deep learning models, I
...
20 Laurence Goasduff, “Why Organizations Choose a Multicloud Strategy,” Gartner, May 7, 2019,
...
You can include arbitrary artifacts such as images, plots, data in nice tabular formats,
...
organization.
24 Michelle Ufford, M. Pacer, Matthew Seal, and Kyle Kelley, “Beyond Interactive: Notebook Innovation at
...
The fourth benefit, which I would argue is the biggest benefit for companies that
...
your fledgling app and suddenly your traffic spikes 10x. You will have to turn on new
...
how to re-create an environment in which your model can run: install this package,
...
a Docker container. You can think of a Dockerfile as the recipe to construct a mold,
...
11.. Download the latest PyTorch base image.
22.. Clone NVIDIA’s apex repository on GitHub, navigate to the newly created apex
folder, and install apex.
33.. Set fancy-nlp-project to be the working directory.
44.. Clone Hugging Face’s transformers repository on GitHub, navigate to the newly
...
can run job A after job B with cron but you can’t schedule anything complicated like
...
11.. Pull last week’s data from data warehouses.
22.. Extract features from this pulled data.
33.. Train two models, A and B, on the extracted features.
44.. Compare A and B on the test set.
55.. Deploy A if A is better; otherwise deploy B.
...
a workflow and schedules each step accordingly. You can even schedule to start a
...
28 Abhishek Verma, Luis Pedrosa, Madhukar Korupolu, David Oppenheimer, Eric Tune, and John Wilkes,
...
quotas (i.e., the maximum number of instances a user can use at a given time), etc.
...
Figure 10-8. After a workflow is defined, the tasks in this workflow are scheduled and
orchestrated
...
Python.
However, like Airflow, containerized steps aren’t the first priority of Prefect. You can
...
to define each step and its requirements in the same file. The following code sample,
...
The main drawback of Argo, other than its messy YAML files, is that it can only run
...
You save on Dockerfiles or YAML files.
...
add @batch decorator to execute it on AWS Batch. You can even run different steps in
...
data center
You’ll need to run and serve your models from a compute layer, and usually tools
...
production?
29 When doing online prediction at a smaller scale, you can just hit an endpoint with payloads and get back
predictions. Batch prediction requires setting up batch jobs and storing predictions.
ML Platform | 321

## Page 342

•• The model is correct, the feature list is correct, but the featurization code is
outdated.
...
A model store that can store sufficient general use cases is far from being a solved
problem. As of writing this book, MLflow is undoubtedly the most popular model
store that isn’t associated with a major cloud provider. Yet three out of the six top
...
30 Neal Lathia, “Building a Feature Store,” December 5, 2020, https://oreil.ly/DgsvA; Jordan Volz, “Why You Need
...
31 Jeremy Hermann and Mike Del Balso, “Meet Michelangelo: Uber’s Machine Learning Platform,” Uber Engi‐
...
code, however, might be written in another language, such as Java or C, for
performance.
...
need is for data movement: moving your data from your applications to your vendor,
...
Your build versus buy decisions depend on many factors. Here, we’ll discuss three
...
company grows, and more and more large companies are looking into repatriating
...
1 Sometimes, you can get different results if you run the same model on the same input twice
at the exact same time.
331

## Page 352

new usage of ML in the real world, how ML systems affect user experience is still not
...
web page as an input to the model, and it’ll output the React code needed to create
...
produce multiple snippets of React code. The code snippets are rendered into visual
...
ny’s code at all. You need cooperation from multiple teams to figure out what’s
wrong.
Finger-pointing
...
The tweet seems to resonate with my audience. Eugene Yan also wrote about how
...
6 “Full Cycle Developers at Netflix—Operate What You Build,” Netflix Technology Blog, May 17, 2018,
https://oreil.ly/iYgQs.
Responsible AI | 339

## Page 360

Responsible AI is the practice of designing, developing, and deploying AI systems
...
7 Elliot Jones and Cansu Safak, “Can Algorithms Ever Make the Grade?” Ada Lovelace Institute Blog, 2020,
...
However, privacy settings and users’ choices only address the problem at a surface
...
know that biases can creep in your system through the entire workflow. Your first
...
implemented by AI Fairness 360 (AIF360). You can also identify hidden bias in
...
23 Michael Feldman, Sorelle Friedler, John Moeller, Carlos Scheidegger, and Suresh Venkatasubramanian, “Certi‐
...
achieved by adding more data. You might also want your model to be fair and
...
25 Eugene Bagdasaryan and Vitaly Shmatikov, “Differential Privacy Has Disparate Impact on Model Accuracy,”
arXiv, May 28, 2019, https://oreil.ly/nrJGK.
26 Sarah Hooker, Aaron Courville, Gregory Clark, Yann Dauphin, and Andrea Frome, “What Do Compressed
...
——Model version
——Model type
29 Jonette M. Stecklein, Jim Dabney, Brandon Dick, Bill Haskins, Randy Lovell, and Gregory Moroney,
...
——Information about training algorithms, parameters, fairness constraints or
...
Model cards are a step toward increasing transparency into the development of ML
...
You might want to create a portfolio of internal tools easily accessible by different
...
algorithms to mitigate bias in datasets and models. You might also consider using
third-party audits.
...
up-to-date with the latest research in responsible AI. You might want to follow the
...
Epilogue
Wow, you made it! You’ve just finished a pretty technical book of 100,000 words and
...
Index
Symbols automated retraining, 275-277
1NF (first normal form), 59 AutoML
2NF (second normal form), 59 architecture search, 174-178
hard AutoML, 174-178
A hyperparameter tuning, 173-174
...
ad hoc analytics, 162 bagging, ensembles, 158-159
adaptability, 31 bandit algorithms, 287-291
adversarial attacks, 272 BASE (basically available, soft state, and even‐
adversarial augmentation, 116 tual consistency), 68
AI (artificial intelligence), ethics, 339, 347-348 base learners, 156
data-driven approach limitations, 349 base model, fine tuning, 100
irresponsible, case studies, 341-347 baselines, offline model evaluation, 179
mitigating biases, 353 existing solutions, 181
model cards, 351-353 human, 180
trade-offs, 349 random, 180
Airflow, 315-316 simple heuristic, 180
alert fatigue, 255, 259 zero rule, 180
alert policies, 259 batch pipeline, 203-205
algorithms batch prediction, 197-201
bandit algorithms, 287-291 moving to online prediction, 201-203
continual learning and, 273-274 batch processing, 78-79
feature importance, 142 batches, overfitting, 167
analytical processing, 67 binary classification, 37
Apache Iceberg, 69 binary data, 57
architectural search, 174 binary file size, 57
Argo, 316-318 boosting, ensembles, 159-161
artifacts, 162 Borg, 314
artificial intelligence (see AI) brand monitoring, 12
asynchronous prediction, 198 browsers, ML (machine learning) and, 222
357

## Page 378

building versus buying, 327-329 feature reuse, 277
business analysis, 35 fresh data access, 270-272
business objectives, 26-28 stateful training, 265-268
automated, 277-278
C stateless retraining, 265-268
manual, 275
calibration, 183-184
...
D
focal loss, 112
challenges, 103-105 DAG (directed acyclic graph), 312
evaluation metrics, 106-108 dashboards, monitoring and, 258
resampling, 109-110 data, 5, 18
class-balanced loss, 112 mind versus data, 43-46
classification training (see training data)
as regression problem, 107 unseen data, 6
binary, 37 data augmentation, 113
hierarchical, 38 adversarial augmentation, 116
high cardinality, 37 data synthesis, 116-117
multiclass, 37, 38 perturbation, 114-116
multilabel, 38 simple label-preserving transformations,
sentiment analysis, 120 114
classification models, 36 data distribution shifts
cloud computing, 212, 300-302 addressing, 248-250
elasticity, 300 detection
multicloud strategy, 302 statistical methods, 243-244
code versioning, 164 time scale windows, 245-247
column deletion, 125 ML system failure, 237
column-major formats, 54-56 concept drift, 238, 241
pandas, 56 covariate shift, 238-240
Parquet, 54 feature change, 241
Commuter, 305 label schema change, 241
compact convolutional filters, 206 label shift, 238, 240
computational priorities, 15 data duplication, data leakage and, 139
compute-intensive problems, 6 data engineering, 34
concept drift, 238, 241 data formats, 53
confidence measurement, 185 binary, 57
containers, 308-310 column-major, 54-56
contextual bandits, 289 JSON, 54
continual learning, 35, 264, 268-270 multimodal data, 53
algorithms and, 273-274 relational model, NoSQL, 63-66
evaluation and, 272-273 row-major, 54-56
358 | Index

## Page 379

text, 57 dependencies, 312
data freshness, model updates and, 279-280 ML models, model store, 322
data generation, data leakage and, 140 dependency failure, 227
data iteration, 267 deployment, 34, 192
model updates and, 281 endpoints, exposing, 192
data leakage, 135 failure, 227
data duplication prior to splitting, 139 ML models, 320
data generation process and, 140 myths
detecting, 140 limited models at once, 194-195
group leakage, 139 model updating, 196
Kaggle competition, 136 performance, 195
scaling before splitting, 138 scale, 196
statistics from test split, missing data and, separation of responsibilities, 193
138 shadow deployment, 282
time-correlated data, 137 development environment, infrastructure, 296,
data models 302
relational, 59-62 containers, 308-310
structured data, 66-67 setup, 303
unstructured data, 66-67 IDE, 303-306
data normalization, 59 standardization, 306-308
data parallelism, distributed training and, directed acyclic graph (DAG), 312
168-170 directional expectation tests, 183
data scientists, teams, 336-339 discretization, feature engineering and, 128-129
data sources, 50 distributed training, 168
databases, internal, 52 data parallelism and, 168
logs, 51 model parallelism and, 170-172
smartphones and, 52 Docker Compose, 310
system-generated data, 50 Docker images, 308-310
third-party data, 52 Dockerfiles, 308-310
user input, 50 document model, 63
data synthesis, 116-117 schemas, 64
data-driven approach, AI ethics and, 349 downtime, 228
databases and dataflow, 72 driver management service, 73
dataflow, 72 dynamic sampling, 110
message queue model, 77
passing through databases, 72 E
...
boosting, 159-161 management, 326
spam classifiers, 157 monitoring, 253-255
stacking, 161 online, 199
ethics in AI, 339-347 reuse, 277
ETL (extract, transform, load), 70-72 streaming, 199
evaluation, offline feedback loops, 288
confidence measurement, 185 ML system failure, 234
directional expectation tests, 183 feedback, users, 93
invariation tests, 182 fixed positional embeddings, 135
model calibration, 183-184 fixed-point inference, 210
perturbation tests, 181-182 FLOPS (floating-point operations per second),
slice-based, 185-188 298
existing data, 5 forecasting customer demand, 11
experiment artifacts, development and, 323 Fourier features, 135
experiment tracking, 162-163 fraud detection, 11, 104
third-party tools, 163
exporting models, 193 G
GDPR (General Data Protection Regulation),
F
164
F1 metrics, 107 generalization, features, 144-146
factorization, low-rank, 206-208 GKE (Google Kubernetes Engine), 314
fairness, 19 Google Translate, 1
feature change, 241 graph model, 65
feature engineering, 120-122
categorical features, 129-132 H
...
building versus buying, 327-329 class imbalance and, 102
cloud computing and, 300-302 errors, class imbalance and, 105
development environment layer, 296, 302 hand labels, 88
setup, 303-306 lineage, 90
fundamental facilities, 295 multiplicity, 89-90
ML platform layer, 296 lack of labels, 94
requirements, 295 active learning, 101-102
resource management layer, 295 semi-supervision, 98-99
storage and compute layer, 295, 296, 297 transfer learning, 99-101
compute resources, 297 weak supervision, 95-98
FLOPS, 298 ML algorithms, 151
private data centers, 300-302 natural labels, 91
public cloud, 300-302 feedback loop length, 92
units, 297 recommender systems, 91
input, monitoring, 255 perturbation, 114-116
instances on-demand, 300 simple label-preserving transformations,
integrated development environment (see IDE) 114
interleaving experiments, 285-287 language modeling, sampling and, 83
internal databases, 52 latency, 16
interpretability, 20 latency versus throughput, 16-18
invariation tests, 182 learning, 3
IR (intermediate representation), 215 LFs (labeling functions), 95
iterative processes heuristics, 95
model development and, 34 logs, 51, 51
performance check, 149 experiment tracking, 162
model updates and, 281 monitoring and, 256-257
training the model and, 32-33 storage, 51
data engineering, 34 loop tiling, model optimization, 218
project scoping, 34 loss curve, 162
loss functions, 40
J (see also objective functions)
low-rank factorization, 206-208
JSON (JavaScript Object Notation), 54
judgment sampling, 83
M
K maintainability, 31
...
system performance, 163 hyperparameters and, 166
mind versus data, 43-46 poor model implementation, 166
missing at random (MAR), 124 random seeds, 167
missing completely at random (MCAR), 124 theoretical constraints, 166
missing data, test split statistics and, 138 iteration, 267
missing not at random (MNAR), 124 monitoring, 35
ML (machine learning) offline evaluation, 178
browsers and, 222, 223 baselines, 179-181
cloud computing, 212-223 methods, 181-188
complex patterns, 4 optimization, 220-221
deep learning and, 1 parameters, model store, 322
edge computing, 212-223 performance metrics, 162
existing data and, 5 selection criteria, 151
learning, 3 human biases in, 153
model optimization, 220-221 model, 155
predictions and, 6 performance now and later, 153
production and, 12-21 simple models, 152
repetition, 7 state-of-the-art trap, 152
research and, 12-21 trade-offs, 154
scale, 7 speed, 163
smartphones and, 9 training, 32-33
unseen data, 6 data engineering, 34
use cases, 9-12 distributed, 168-172
when to use, 3-12 update frequency, 279
ML algorithms, 2, 149 data freshness and, 279-280
deep learning and, 150 data iteration and, 281
labels, 151 model iteration and, 281
versus neural networks, 150 updates, 267
ML model logic, 191 versioning, 163-165
ML models ML platform, 319
continual learning, 35 model deployment, 320
data iteration, 267 model store, 321-325
debugging, 165 ML platform layer, infrastructure, 296
deployment, 320 ML system failures
edge computing, optimization, 214-221 data distribution shifts, 237
ensembles, 156, 157 addressing, 248-250
bagging, 158-159 concept drift, 238, 241
base learners, 156 covariate, 238-240
boosting, 159-161 detection, 242-247
stacking, 161 feature change, 241
evaluation, 150 label schema change, 241
test in production, 281-291 label shifts, 238, 240
experiment tracking, 162-163 ML-system specific
exporting, 193 degenerate feedback loops, 233-236
failures edge cases, 231
batches, overfitting, 167 production data different from training
components, 167 data, 229-231
data problems, 166 operational expectation violations, 227
feature choice, 166 software
362 | Index

## Page 383

crashes, 228 recommender systems, 91
dependency failure, 227 natural language processing (NLP) (see NLP)
deployment failure, 227 neural architecture search (NAS), 174
downtime, 228 neural networks, 150
hardware failure, 228 positional embedding, 133
ML systems newsfeeds
declarative, 62 ranking posts, 41
failures, 226 user engagement and, 41
iterative processes, 32-35 NLP (natural language processing), 114
requirements data augmentation and, 113
adaptability, 31 feature engineering, 122
maintainability, 31 nonprobability sampling, 83
reliability, 29 biases, 83
scalability, 30-31 Norvig, Peter, 44
versus traditional software, 22-23 NoSQL, 63
MLOPs, ML systems design and, 2-3 document model, 63
MNAR (missing not at random) values, 124 graph model, 65
model biases, AI ethics, 347-348 notebooks, IDE and, 304
model calibration, 183-184 NSFW (not safe for work) content filtering, 41
model cards, AI ethics, 351-353 NumPy, 56
model compression, 206
knowledge distillation, 208 O
low-rank factorization, 206-208
...
n-grams, 120 orchestrators
NAS (neural architecture search), 174 HashiCorp Nomad, 314
natural labels, 91 Kubernetes (K8s), 314
feedback loop length, 92 outliers, edge cases and, 232
Index | 363

## Page 384

oversampling pruning, 208-209
overfitting, 110 public cloud versus private data center, 300-302
SMOTE, 110
Q
P
quantization, 209-211
pandas, 56 query languages, 60
Papermill, 305 quota sampling, 83
parallelization, model optimization and, 217
parameter values over time, 163 R
...
row-major format, 54-56 smooth failing, user experience, 334
CSV (comma-separated values), 54 SMOTE (synthetic minority oversampling
NumPy, 56 technique), 110
RPC (remote procedure call), 74 Snorkel, 95
snowball sampling, 83
S soft AutoML, 173-174
...
T lack of labels, 94-102
tags, model store, 323 natural labels, 91-94
tasks user feedback, 93
classification, 36 n-grams, 121
binary, 37 noisy samples, 116
high cardinality, 37 sampling, 82
multiclass, 37, 38 importance sampling, 87
multilabel, 38 nonprobability, 83-84
labels, 91 reservoir sampling, 86-87
regression, 36, 39 simple random sampling, 84
teams stratified sampling, 84
cross-functional collaboration, 335 weighted sampling, 85
data scientists, 336-339 training the model, iteration and, 32-33
production management, 336 data engineering, 34
telemetry, 260 project scoping, 34
test in production, 263, 281 transactional processing, 67
A/B testing, 283-284 ACID and, 68
bandits, 287-291 transfer learning, 99-101
canary release, 285 two-phase learning, 110
interleaving experiments, 285-287
U
shadow deployment and, 282
text data, 57 undersampling, 109
text file size, 57 unseen data, 6
theoretical constraints, failures and, 166 unstructured data, 66-67
third-party data, 52 updates, deployment myths, 196
time-correlated data, data leakage and, 137 use cases, 9-12
training user experience, 331
automated retraining, 275-277 consistency, 332
distributed, 168 predictions, mostly correct, 332-334
data parallelism and, 168-170 smooth failing, 334
model parallelism and, 170-172 user feedback, 93
stateful, 265-268 user input data, 50
automated, 277-278
stateless retraining, 265-268 V
manual, 275
...
W
resampling, 109-110
data augmentation, 113 WASM (WebAssembly), 223
perturbation, 114-116 weak supervision, 95-98
simple label-preserving transformations, Snorkel, 95
114 weighted sampling, 85
data distributions, 229 word embeddings, 133
data leakage, 135 workflow management, 314
labeling, 88 Airflow, 315-316
hand labels, 88-90 Argo, 316-318
366 | Index

## Page 387

DAG (directed acyclic graph), 312 Z
Kubeflow, 318 zero rule baselines, 180
Metaflow, 318 zero-shot learning, 100
X
XGBoost, 142
Index | 367
